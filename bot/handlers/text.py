...
@router.message(F.text & ~F.text.startswith("/") & ~F.text.in_({"🏠 Меню", "📚 Словник", "🎙 Вимова"}))
async def handle_text(message: Message, state: FSMContext, db_user: User, **kwargs) -> None:
    try:
        ...
        elif current_state in (VocabularyStates.srs_review.state, VocabularyStates.training.state):
            from bot.handlers.vocabulary import handle_vocab_answer
            await handle_vocab_answer(message, text, state, db_user)
        ...
        elif current_state == OnboardingStates.awaiting_first_voice.state:
            from bot.handlers.translate import handle_translate_onboarding
            await handle_translate_onboarding(message, text, state, db_user)
        else:
